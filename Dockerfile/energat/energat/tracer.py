...
#            #        continue
#
#            is_thread = any(t.id in self.target_threads for t in status.target.threads())
#            if is_thread:
#                if any(t.id in ascribed_threads for t in status.target.threads()):
#                    # * Dedupe: Threads share memory with other threads of the same process group.
#                    continue
#
#            for socket in range(self.num_cpu_sockets):
...
#        return ascribable_energy_j, credit_fracs, tracer_energy_j
    ...
    def ascribe_energy(
        ...
        :param total_energy_j: [[pkg1, pkg2, ...], [mem1, mem2, ...]]
        :param total_server_cputime_sec: [socket1, socket2, ...]
        :return: (ascribable_energy_j, credit_fracs, tracer_energy_j)
        """
        is_tracer = lambda _id: _id in [
            self.tracer_process.pid,
        ...
        if not self.targets_status:
            # No active targets.
            return ascribable_energy_j, self.get_empty_energy_readings(), tracer_energy_j

        # Lock so daemon won't mutate samples mid-aggregation
        self.mutex.acquire()

        try:
            # ----- Determine a robust num_mem_samples -----
            num_mem_samples = 0
            # server samples first
            for socket in range(self.num_cpu_sockets):
                num_mem_samples = max(num_mem_samples, len(self.server_numa_mem_samples[socket]))
            # target samples (may be empty for some targets)
            for status in self.targets_status.values():
                for socket in range(self.num_cpu_sockets):
                    num_mem_samples = max(
                        num_mem_samples,
                        len(status.numa_mem_samples.get(socket, []))
                    )

            # If still zero, no one has samples yet -> skip this lap gracefully
            if num_mem_samples == 0:
                credit_fracs = self.get_empty_energy_readings()
                return ascribable_energy_j, credit_fracs, tracer_energy_j

            # ----- Prepare accumulators with the decided length -----
            ascribable_cputime = [0.0] * self.num_cpu_sockets
            tracer_cpu = [0.0] * self.num_cpu_sockets

            accumulated_private_mem_samples = {
                socket: np.zeros(num_mem_samples, dtype=float)
                for socket in range(self.num_cpu_sockets)
            }
            tracer_mem_samples = {
                socket: np.zeros(num_mem_samples, dtype=float)
            ...
            # ----- Aggregate per-target CPU + DRAM samples -----
            for _, status in self.targets_status.items():
                if not target_exists(status.target.pid):
                    continue
                ...
                # CPU attribution
                target_cputime = status.cputime_delta
                socket_residence_probs = status.compute_socket_residence_probs(self.num_cpu_sockets)
                for socket in range(self.num_cpu_sockets):
                    cputime = target_cputime * socket_residence_probs[socket]
                    if is_tracer(status.target.pid):
                        tracer_cpu[socket] += cputime
                ...
                # DRAM attribution
                # Treat missing/short per-target samples as zeros/padded
                # (threads dedupe logic preserved)
                is_thread = any(t.id in self.target_threads for t in status.target.threads())
                if is_thread:
                    if any(t.id in ascribed_threads for t in status.target.threads()):
                        continue
                ...
                for socket in range(self.num_cpu_sockets):
                    # target series
                    ts = status.numa_mem_samples.get(socket, [])
                    arr = np.asarray(ts, dtype=float)
                    if arr.size >= num_mem_samples:
                        arr = arr[-num_mem_samples:]
                    else:
                        # pad at head with zeros to align lengths
                        arr = np.pad(arr, (num_mem_samples - arr.size, 0), mode="constant", constant_values=0.0)

                    if is_tracer(status.target.pid):
                        tracer_mem_samples[socket] += arr
                    else:
                        accumulated_private_mem_samples[socket] += arr

                if is_thread:
                    siblings = {t.id for t in status.target.threads()}
                    ascribed_threads.update(siblings)

            # ----- Compute credit fractions and energies -----
            SMALL_CONST = 1e-5
            credit_fracs = self.get_empty_energy_readings()
            ascribable_cputime = np.array(ascribable_cputime)
            ...
            for socket in range(self.num_cpu_sockets):
                ...
                # CPU credit
                gamma_cpu = FLAGS.gamma
                denom = total_server_cputime_sec[socket]
                cpu_credit_frac = min(1.0, ascribable_cputime[socket] / denom) if denom > 0 else SMALL_CONST
                ascribable_cpu_j = cpu_energy * (cpu_credit_frac ** gamma_cpu)
                ascribable_energy_j[0][socket] = ascribable_cpu_j
                credit_fracs[0][socket] = cpu_credit_frac

                # DRAM credit
                server_mem_samples = np.asarray(self.server_numa_mem_samples[socket], dtype=float)
                if server_mem_samples.size >= num_mem_samples:
                    server_mem_samples = server_mem_samples[-num_mem_samples:]
                else:
                    # pad by repeating last known value or SMALL_CONST if empty
                    pad_val = server_mem_samples[-1] if server_mem_samples.size > 0 else SMALL_CONST
                    server_mem_samples = np.pad(
                        server_mem_samples, (num_mem_samples - server_mem_samples.size, 0),
                        mode="constant", constant_values=pad_val
                    )

                # avoid zeros to keep ratios stable
                mem_indices_zeros = np.where(server_mem_samples == 0)
                if mem_indices_zeros[0].size > 0:
                    server_mem_samples[mem_indices_zeros] = SMALL_CONST

                priv = accumulated_private_mem_samples[socket].copy()
                priv[mem_indices_zeros] = np.maximum(priv[mem_indices_zeros], SMALL_CONST)

                delta = FLAGS.delta
                mem_credit_frac = min(1.0, (priv / server_mem_samples).mean())
                ascribable_dram_j = dram_energy * (mem_credit_frac ** delta)
                ascribable_energy_j[1][socket] = ascribable_dram_j
                credit_fracs[1][socket] = mem_credit_frac

                # Tracer fractions (for accounting)
                tracer_cpu_frac = min(1.0, tracer_cpu[socket] / denom) if denom > 0 else SMALL_CONST
                tracer_energy_j[0][socket] = cpu_energy * (tracer_cpu_frac ** gamma_cpu)

                tracer_series = tracer_mem_samples[socket]
                tracer_mem_frac = min(1.0, (tracer_series / server_mem_samples).mean())
                tracer_energy_j[1][socket] = dram_energy * (tracer_mem_frac ** delta)

                if round(time.time()) % FLAGS.logging == 0:
                    logger.debug(f"{socket=}: {cpu_credit_frac=: .3f}, {mem_credit_frac=: .3f}")
                    logger.debug(f"{socket=}: {tracer_cpu_frac=: .3f}, {tracer_mem_frac=: .3f}")
...
#            )
        ...
        root_pid = self.target_process.pid
        root_alive = target_exists(root_pid)

        # If root is gone, either stop (default) or linger and salvage previous targets.
        if not root_alive:
            logger.warn(f"Target application ({root_pid}) appears to have exited.")
            if not FLAGS.linger_on_root_exit:
                return False
            # Linger mode: keep tracing any previously tracked PIDs that are still alive.
            for pid in list(self.target_processes):
                if target_exists(pid):
                    processes.add(pid)
            # In process-only mode we intentionally ignore per-thread targets.
            if not FLAGS.process_only_targets:
                for tid in list(self.target_threads):
                    if target_exists(tid):
                        threads.add(tid)
            # NEW: even while lingering, continue to discover descendants of the
            #      currently tracked processes so we don't miss new children.
            for parent_pid in list(processes):
                try:
...
#            processes.add(self.target_process.pid)
        ...
        if root_alive:
            if self.target_process.num_threads() > 1:
                ...
                if FLAGS.process_only_targets:
                    # Attribute at process granularity
                    processes.add(self.target_process.pid)
                else:
                    for thread in self.target_process.threads():
                        # NB: main thread is included
                        threads.add(thread.id)
            else:
                processes.add(self.target_process.pid)
...
#                logger.warn(f"{child_process.pid=} has gone")
        ...
        # When root is alive, walk its children as before.
        if root_alive:
            for child_process in self.target_process.children(recursive=True):
                try:
                    if child_process.status() in inadmissible_status:
...
#                        threads.add(tid)
#
#            # NEW: while lingering, continue to discover descendants of the
#            # currently tracked processes so we don't miss new children.
#            for parent_pid in list(processes):
#                try:
#                    parent_proc = psutil.Process(parent_pid)
...
#                    try:
#                        if child_proc.status() in inadmissible_status:
#                            logger.warn(f"{child_proc.pid=}: {child_proc.status()}")
#                            continue
#
#                        if FLAGS.process_only_targets:
#                            if child_proc.pid not in processes:
#                                processes.add(child_proc.pid)
#                                logger.info(f"(linger) Added {child_proc.pid=} (child of {parent_pid})")
#                        else:
#                            if child_proc.num_threads() > 1:
#                                for th in child_proc.threads():
#                                    if th.id not in threads:
#                                        threads.add(th.id)
#                                        logger.info(f"(linger) Added thread {th.id=} (child of {child_proc.pid})")
#                            else:
#                                if child_proc.pid not in processes:
#                                    processes.add(child_proc.pid)
#                                    logger.info(f"(linger) Added {child_proc.pid=} (child of {parent_pid})")
...
#
#        else:
#            # Root is alive: sanity check and then add root + its children (original behavior).
#            if self.target_process.status() in inadmissible_status:
#                raise RuntimeError(f"Target application status: {self.target_process.status()}")
#
#            # Multithreaded main target.
#            if self.target_process.num_threads() > 1:
#                logger.info(f"{self.target_process.pid=}: {self.target_process.num_threads()=}")
#                if FLAGS.process_only_targets:
#                    processes.add(self.target_process.pid)
#                else:
#                    for thread in self.target_process.threads():
...
#                processes.add(self.target_process.pid)
#
#            # Walk descendants of the root (as before).
#            for child_process in self.target_process.children(recursive=True):
#                try:
#                    if child_process.status() in inadmissible_status:
#                        logger.warn(f"{child_process.pid=}: {child_process.status()}")
