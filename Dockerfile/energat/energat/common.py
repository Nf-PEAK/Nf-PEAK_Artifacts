...
flags.DEFINE_float("gamma", 0.3, "Non-linear scaling factor for CPU power") #Original: 0.3
#flags.DEFINE_float("gamma", 0.22, "Non-linear scaling factor for CPU power") #Test gamma is an exponent of a fraction -> number bigger: result smaller
flags.DEFINE_float("delta", 0.2, "Non-linear scaling factor for DRAM power")
#flags.DEFINE_float("delta", 0.68, "Non-linear scaling factor for DRAM power") #Test; delta is an exponent of a fraction -> number bigger: result smaller
flags.DEFINE_float(
...
# ===== Attribution behavior knobs (default off: preserves current behavior) =====
# If true, attribute per **process** only (no per-thread targets).
flags.DEFINE_boolean(
    "process_only_targets", False,
    "If true, ignore per-thread targets; attribute per process only."
)

# If true, keep tracing even after the root PID exits; stop only when no targets remain.
flags.DEFINE_boolean(
    "linger_on_root_exit", False,
    "If true, do not stop immediately when the root PID exits; "
    "continue until no targets remain."
)
...
def read_cputime_sec(pid: int) -> float:
    """Get CPU time in seconds for a given ID.
    - If `pid` is a **process PID**, read /proc/<pid>/stat  (aggregated over all threads).
    - If `pid` is a **thread TID** (no /proc/<pid>/stat), fall back to /proc/<pid>/task/<pid>/stat.
    """
    # Prefer process-aggregated stat if present
    statf_proc = f"/proc/{pid}/stat"
    statf_task = f"/proc/{pid}/task/{pid}/stat"
    statf = statf_proc if os.path.isfile(statf_proc) else statf_task
    ...
    with open(statf, "r") as f:
        stat = (f.read().rstrip("\n")).split()
        # Fields 14 & 15 are utime/stime (clock ticks)
        num_clock_ticks = int(stat[14 - 1]) + int(stat[15 - 1])
    return num_clock_ticks / CLK_TCK_PER_SEC
...
def target_exists(pid: int) -> bool:
    # Consider both process and thread cases
    return os.path.isfile(f"/proc/{pid}/stat") or os.path.isfile(f"/proc/{pid}/task/{pid}/stat")
...
def pin_tasks(tasks: List[int], cores: List[int] = None, n_cores: int = 1):
    ...
    for task in tasks:
        subprocess.run(
            ["taskset", "-cp", ",".join(map(str, cores)), f"{task}"], check=True
        )
...
class StdRapl(object):
    ...
    def run_std_rapl(self):
        total_energy = float(
            subprocess.check_output(["./uarch_rapl", "-t", "1000"]).decode()[
                :-1
            ]
        )
